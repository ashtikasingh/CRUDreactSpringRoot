...
export default function EditUser() {
  ...
  const onSubmit= async(e)=>{
    e.preventDefault();
    await axios.put(`http://localhost:8880/user/${id}`, user);
    navigate("/");
  };

  const loadUser = async () => {
    const result = await axios.get(`http://localhost:8880/user/${id}`);
    setUser(result.data);
  }
  return (
    <>
        ...
        <div className='row'>
          <div className='col-md-6 offset-md-3 border rounded p-4 mt-2 shadow'>
            ...
            <div className='mb-3'>
              ...
              <input type={"email"} className='form-control' placeholder='Enter your e-mail' name='email' value={email} onChange={(e)=>onInputChange(e)}/>
            </div>
